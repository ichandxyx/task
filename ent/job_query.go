...
func (jq *JobQuery) QueryVisits() *VisitQuery {
	query := &VisitQuery{config: jq.config}
...
func (jq *JobQuery) Clone() *JobQuery {
	...
	return &JobQuery{
		...
		withVisits: jq.withVisits.Clone(),
...
func (jq *JobQuery) WithVisits(opts ...func(*VisitQuery)) *JobQuery {
	query := &VisitQuery{config: jq.config}
...
func (jq *JobQuery) sqlAll(ctx context.Context) ([]*Job, error) {
	...
	if query := jq.withVisits; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Job)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Visits = []*Visit{}
		}
		query.withFKs = true
		query.Where(predicate.Visit(func(s *sql.Selector) {
			s.Where(sql.InValues(job.VisitsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.job_visits
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "job_visits" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "job_visits" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Visits = append(node.Edges.Visits, n)
		}
	}
